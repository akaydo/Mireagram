...
router.post(
  ...
  async (req, res) => {
    ...
    if (user) {
      return res.status(200).json("Учетная запись уже создана");
    }
    ...
    transport.sendMail({
      ...
      html: `<h1>Ваш код подтверждения: ${OTP}</h1>`,
    });
    res.status(200).json({
      Status: "Pending",
      msg: "Проверьте почту для подтверждения регистрации.",
      user: user._id,
    });
...
router.post("/verify/email", async (req, res) => {
  ...
  if (!mainuser) return res.status(400).json("Пользователь не найден.");
  if (mainuser.verifed === true) {
    return res.status(400).json("Учетная запись уже подтверждена.");
  }
  const token = await VerificationToken.findOne({ user: mainuser._id });
  if (!token) {
    return res.status(400).json("Токен не найден.");
  }
  const isMatch = await bcrypt.compareSync(OTP, token.token);
  if (!isMatch) {
    return res.status(400).json("Токен не валиден.");
  }
  ...
  transport.sendMail({
    ...
    html: `Теперь вы можете авторизоваться на сайте`,
  });
...
router.post("/forgot/password", async (req, res) => {
  ...
  transport.sendMail({
    ...
    to: user.email,
    subject: "Reset Token",
    html: `https://mireagram.vercel.app/reset/password?token=${RandomTxt}&_id=${user._id}`,
  });
...
router.put("/following/:id", verifyToken, async (req, res) => {
  if (req.params.id !== req.body.user) {
    ...
    if (!user.Followers.includes(req.body.user)) {
      ...
      return res.status(200).json("Вы подписаны на пользователя");
    } else {
      await user.updateOne({ $pull: { Followers: req.body.user } });
      await otheruser.updateOne({ $pull: { Following: req.params.id } });
      return res.status(200).json("Вы отписаны от пользователя");
    }
  } else {
    return res.status(400).json("Вы не можете подписаться на себя");
  }
});
